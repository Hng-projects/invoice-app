...
export function useInvoiceForm(
  ...
  onSave: ((invoice: InvoiceItemProps) => void) | undefined
) {
  ...
  const handleAddItem = () => {
    setItems([...items, { id: Date.now().toString(), name: "", qty: "1", price: "0.00", total: 0 }]);
  };
  ...
  const saveInvoice = (statusToSave: "draft" | "pending") => {
    ...
    const clientName = (formData.get("clientName") as string) || (statusToSave === "draft" ? "Draft" : "New Client");
